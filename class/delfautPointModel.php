<?php 
	...
	class delfaultPointModel extends pointModel{

		const DELFAULTPARAMS = ['./points_models/delfaut/delfaut.png', '41px', '41px', null, null, null, null, null, null, null, null, null];
		...
		public function drawPointModel (&$pPoint){
			...
			$svgText = '<g  id="'.$pPoint->getID().'" onclick="viewInfo(\''.$pPoint->getID().'\');" ><image '.$pPoint->getXMLPos().' xlink:href="'.$params[self::IMAGEPOS].'" height="'.$params[self::HEIGHTPOS].'" width="'.$params[self::WIDTHPOS].'" viewbox="'.$pPoint->getX().' '.$pPoint->getY().' '.$params[self::WIDTHPOS].' '.$params[self::HEIGHTPOS].'" preserveAspectRatio="xMidYMid Slice" />';
